...
see auto-generated documentation [here](https://drift-labs.github.io/protocol-v2/sdk/)
...
| programId              | Drift program id                                                                                                                                                                 | Yes      | `dRiftyHA39MWEi3m9aunc5MzRF1JYuBsbn6VPcn33UH` ([Solscan](https://solscan.io/account/dRiftyHA39MWEi3m9aunc5MzRF1JYuBsbn6VPcn33UH)) |
...
```typescript
const [txSig, userPublickKey] = await driftClient.initializeUser(0, "toly");
```
...
await this.driftClient.updateUserMarginTradingEnabled(true, subaccountId);
...
driftClient.deposit(amount, marketIndex, associatedTokenAccount);
```
...
driftClient.withdraw(amount, marketIndex, associatedTokenAccount);
```
...
const orderParams = {
  ...
  auctionStartPrice: driftClient.convertToPricePrecision(21.2),
  auctionEndPrice: driftClient.convertToPricePrecision(21.3),
...
const baseAssetAmount = user.getPerpPosiiton(marketIndex)?.baseAssetAmount;
...
const order = user.getOrderByUserOrderId(userOrderId);
```
